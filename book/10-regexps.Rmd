...
`s/regexp/replacement/` which is like a search (regexp) and replace (replacement part) command, but
more powerful. In the replacement part, you can refer to the whole match (`&`) or parts of the match
that were captured with parentheses (`\1`). After the last `/` you can add modifiers; the most
commonly used is `g` (global) which does more than one replacement per line.
