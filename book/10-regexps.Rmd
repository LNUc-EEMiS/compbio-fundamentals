...
R has a very complete regexp engine. One primarily uses the `grep` family of functions for regexps
in R: `grep` and `grepl` for matching, i.e. searching, `sub` and `gsub` for substituation (the
former does *one* substituation, the latter global, i.e. as many substitutions as possible) and
...
Some examples, in a Tidyverse setting:
