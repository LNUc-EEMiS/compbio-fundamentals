...
*C*entered *L*og *R*atios (CLRs) are calculated as log of the ratio between a count and the
geometric mean of counts in the sample:

$$
clr_s = \log{\frac{c_s}{\sqrt[n]{\prod_{i = 1}^{n} c_i}}}
$$

Where $n$ is the number of samples, $cls_s$ is the centered log ratio and $c_s$ the count
...
samples <- read_tsv(
  ...
  # Subset the table to only contain the samples that are in the asvs table (which was subset
  # to only contain samples > 500 observations).
...
Centered log ratios do not sum to the same sum for all samples -- in fact, they sum to many orders
of magnitude different values `r figr('clrsum-plot', T, type = 'Figure')` -- and they are often
negative (beacuse one takes the log). They are hence not good replacements for relative abundances
in e.g. heatmaps.
...
of the environmental parameters. (Usually categorical variables are plotted as points and not as
arrows.)

```{r rda-triplot, fig.height = 6}
...
rda$CCA$u %>% data.frame() %>% tibble::rownames_to_column('sample') %>%
  ...
  ggplot(aes(x = RDA1, y = RDA2)) +
    ...
    geom_segment(data = a, mapping = aes(xend = RDA1/4, yend = RDA2/4), x = 0, y = 0, arrow = arrow()) +
    geom_text(data = a, mapping = aes(x = RDA1/8, y = RDA2/8, label = factor), size = 4)
```