...
*C*entered *L*og *R*atios (clrs) are calculated as the ratio between a count and the geometric mean of counts in the sample:

$$
clr_s = \frac{c_s}{\sqrt[n]{\prod_{i = 1}^{n} c_i}}
$$

Where $n$ is the number of samples, $cls_s$ is the centered log ratio and $c_s$ the count respectively for sample $s$.

The product in the denominator will be zero if there's a single zero, i.e. unobserved taxon (ASV) in a sample. The root
...
function in the `zCompositions` package...
...
Centered log ratios do not sum to the same sum for all samples `r figr('clrsum-plot', T, type = 'Figure')`, they are hence
not good replacements for relative abundances.
